...
async def build_and_run(cfg) -> int:
    ...
    # Do this first: it takes ~10 s cold, and doing it here rather than on the
    # user's first sentence is the difference between "instant" and "broken".
    warm_up_llm(cfg.get("llm.base_url", "http://localhost:11434/v1"),
                cfg.get("llm.model", "gemma3:4b"),
                cfg.get("llm.keep_alive", "60m"))
    ...
    if cfg.get("vision.enabled", True):
        camera = Camera(
            ...
            poll_fps=cfg.get("camera.poll_fps", 5),
        )
        if not camera.start():
    ...
    stt = StrictWhisperSTTService(
        settings=WhisperSTTService.Settings(model=cfg.get("stt.model", "base")),
        device=cfg.get("stt.device", "cpu"),
        compute_type=cfg.get("stt.compute_type", "int8"),
        # Whisper's own confidence that a segment contains no speech. Above this,
        # the segment is discarded rather than hallucinated into words. Essential
        # with `tiny`, which invents text from silence.
